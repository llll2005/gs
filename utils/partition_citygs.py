...
# Use a non-interactive backend so partition plots are saved to file (partitions.png) but no
# preview window pops up. Must be set before pyplot is first imported (via citygs_partitioning_utils).
import matplotlib
matplotlib.use("Agg")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--partition_subdir", type=str, default=None,
                        help="extra sub-dir under partition/ (e.g. config name) so different coarses/runs "
                             "with the same block_dim/threshold do not overwrite each other's partition")
    ...
    if args.config_path is not None:
        ...
        args.block_dim = getattr(params, "block_dim", args.block_dim)
        args.partition_subdir = getattr(params, "partition_subdir", args.partition_subdir)
        args.content_threshold = params.content_threshold if hasattr(params, "content_threshold") else args.content_threshold
        if args.contract:
            args.aabb = params.aabb if hasattr(params, "aabb") else args.aabb
        
        ckpt_path = getattr(config.model, "initialize_from", None) or args.model_path
        # Accept either a checkpoint file OR a directory: search_load_file auto-finds the
        # latest ckpt inside <dir>/checkpoints/, so the config can point to the coarse output
        # dir without hardcoding epoch/step (matches the fit path's behaviour).
        ckpt_path = GaussianModelLoader.search_load_file(ckpt_path)
        if "point_cloud" in ckpt_path:
            args.model_path = ckpt_path.split("/point_cloud/")[0]
        elif "checkpoints" in ckpt_path:
            args.model_path = ckpt_path.split("/checkpoints/")[0]
    ...
    # build_output_dirname() = "partition/partitions-dim_X_Y_visibility_T".
    # Optionally insert a sub-dir (e.g. config name) between "partition" and "partitions-dim_..."
    # so different coarses/runs with the same block_dim/threshold do not overwrite each other.
    partition_dirname = scene.build_output_dirname()
    if args.partition_subdir:
        _head, _tail = os.path.split(partition_dirname)  # ("partition", "partitions-dim_...")
        partition_dirname = os.path.join(_head, args.partition_subdir, _tail)
    output_path = os.path.join(dataset_path, partition_dirname)
