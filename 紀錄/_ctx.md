...
正文是 **`研究總覽_v2.md`**（2026-09-13 重寫，§0 是一頁現況）；舊版 `研究總覽.md` 尚未刪，只在 v2 沒涵蓋處參考。
這份只放「開場就該知道、且查一次要花很多 token」的東西。**有衝突以 `研究總覽_v2.md` 為準。**
更新本檔時只增刪要點，不要寫敘事。

## ⛔⛔ 2026-09-17 現況（**本節取代下方所有較舊的「最強活線／現行最佳／未測」敘述**；細節＝v2 §0）

- **2026-09-13 第三次分界：影像↔姿態錯位** ⇒ 本機 `outputs/` 的訓練分數作廢；新年代數字在 `outputs/lab/`。
- **訓練一律在 lab**（使用者 2026-09-17）；lab 已比過的不要自己換條件重跑；沒被要求的新實驗只提案。
- ★★ trim 判準 v -> v/c：短配方（22k~27k，三塊）同 N、Load -64~-67%、LPIPS 較好、PSNR -0.00~-0.17；
  ⚠⚠ **60k 完整配方 PSNR -0.27 / -0.54**（b6/b13）⇒ 品質代價隨訓練長度放大；60k 成本欄未量。=> v2 §1.5
- Elongation（60k）：硬剪 -0.15/-0.23；搬移平手（首次觸發只命中 0~1 顆）；**trimvpc+硬剪 -0.68/-0.95** ⇒ 不互補。=> v2 §1.5
- 約束端：閘門比**最大值**是 b12 退化的根因（b12 參考軌跡證實）；相對預算三組（b6）前鬆後緊最划算；
  擋加點擋不住每顆變大（足跡 2.5~3.4 倍）。=> v2 §1.2
- init（20k 機制全關）：sfmfill > sfm（+0.17/+0.19）> depth > random；sfmfill 參數比較（**更正版**，基準 0.35/0.35）跑中。=> v2 §4
- 官方 CityGaussianV2：aerial 的 trim 階段**從未 trim**（ckpt 覆蓋 renderer，`CITYGS_KEEP_CFG_RENDERER=1` 已修）；
  共用深度圖是 1920x1080（官方要 -d 1.2）；官方參考線（未修改原始碼、獨立資料目錄 `block_all_official`）已排 lab。
- 短跑煙霧測試：trim 要 `STEPS/2 >= 1000`；densify 類旗標最少 STEPS=2102，用 2400。
- lab 操作（排程／拉取／腳本地圖）＝`紀錄/完整指令手冊.md` §11-4／§11-5／§11-6；查磁碟要打在 `/workspace/data/hdd`。
...
## ⛔【第三次分界作廢，見最上方現況】舊活線：SfM-init（2026-09-10）
...
⛔ 否證  「c_i 引導**取樣**會提升品質」
✅ 已測（2026-09-13）c_i 放進**約束**（`cost_budget` 取代 cap_max）—— **會漏**，見最上方現況
