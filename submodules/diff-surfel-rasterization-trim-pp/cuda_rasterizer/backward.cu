	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (rho3d <= rho2d) {
				...
				// [ABSGRAD 2026-08-25] AbsGS (arXiv 2404.10484 §3.2) 的 2DGS 對應物。
				// 原文證明：3DGS 的 screen-space 位置梯度 sum_p dL/dmu 會因為 splat 中心兩側
				// 的像素推向相反方向而互相抵消（梯度碰撞），所以過度重建的大高斯永遠達不到
				// 分裂門檻；修法是**先取絕對值再求和**。
				// 2DGS 走 ray-splat 路徑（rho3d <= rho2d，即解析良好的粒子）時，位置訊號是
				// `dL_ds`＝「loss 想把 ray-splat 交點移到 splat 上的別處」，與 AbsGS 微分的
				// 是同一個物件，只是用 splat 局部座標表示；它同樣會抵消。而它立刻被轉成
				// dL_dTu/Tv/Tw 埋進 9 個 transMat 分量、與旋轉/尺度梯度混在一起
				// => 逐顆的位置訊號目前無處可取。
				// `dL_dmean2D.z` 全檔從未被讀（梯度鏈只用 .x/.y，見本檔 629-632）
				// => 借它當累加器，不影響任何梯度、不需新 buffer、不改 API。
				// ⚠ 刻意**只**在此分支累加：else 分支是次像素 fallback，其梯度是螢幕座標，
	...
	glm::vec4* dL_drots)
{
	...
	float4 intrins = {focal_x, focal_y, focal_x * tan_fovx, focal_y * tan_fovy + pp_shifty};
