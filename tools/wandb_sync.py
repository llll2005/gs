...
def main():
    ...
    jobs = [f for f in os.listdir(args.output_path) if os.path.isdir(os.path.join(args.output_path, f))]

    with ProcessPoolExecutor(max_workers=len(jobs)) as executor:
        futures = [executor.submit(sync, os.path.join(args.output_path, job, 'wandb/latest-run')) for job in jobs]
