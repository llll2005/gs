...
class ColmapBlockDataParser(ColmapDataParser):
    ...
    def get_outputs(self) -> DataParserOutputs:
        ...
        if self.params.block_id is not None:
            ...
            # Optional sub-dir (e.g. config name) under partition/ — must match what
            # partition_citygs.py wrote (its --partition_subdir / config partition_subdir).
            partition_root = os.path.join(self.path, "partition")
            if getattr(self.params, "partition_subdir", None):
                partition_root = os.path.join(partition_root, self.params.partition_subdir)
            self.params.image_list = os.path.join(partition_root, "partitions-dim_{}_{}_visibility_{}".format(
            ...
            for i in images:
                ...
                if image.name in selected_image_names:
                    ...
                    new_images[i] = image
            if len(new_images) <= self.params.min_images_per_block:
                raise RuntimeError(
                    f"Block has only {len(new_images)} images (min_images_per_block={self.params.min_images_per_block}). "
                    f"Lower min_images_per_block in config or check partition file: {self.params.image_list}"
                )
            if len(new_images) < 50:
                print(f"[WARNING] Block has only {len(new_images)} images — risk of overfitting ({self.params.image_list})")
        ...
        for idx, key in enumerate(images):
            ...
            appearance_id_list.append(image_name_to_appearance_id[extrinsics.name])
            normalized_appearance_id_list.append(image_name_to_normalized_appearance_id[extrinsics.name])
