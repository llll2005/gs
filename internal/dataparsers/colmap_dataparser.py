...
class ColmapDataParser(DataParser):
    ...
    def get_image_dir(self) -> str:
        if self.params.image_dir is None:
            ...
            # Robust fallback: the 4-digit->6-digit rename created `input/` (6-digit) but the
            # default `images[_N]` dir may be a stale/missing symlink folder. If the resolved
            # dir is absent, fall back to `input/` (the per-image zfill fallback in the image
            # resolver below then maps COLMAP's 4-digit names to the 6-digit files on disk).
            # Without this, test runs that don't set image_dir resolve to a missing dir and
            # every GT image is "not found" -> black/blank eval images.
            if not os.path.isdir(image_dir):
                for alt in ("input", "images"):
                    alt_dir = os.path.join(self.path, alt)
                    if os.path.isdir(alt_dir):
                        print(f"[colmap dataparser] image_dir '{image_dir}' missing -> falling back to '{alt_dir}'")
                        return alt_dir
            return image_dir
