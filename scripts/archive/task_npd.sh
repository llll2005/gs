#!/bin/bash
# Both geometric priors off AND lambda_dssim 0.5 -- the two strongest changes, never combined.
#
#                     PSNR    SSIM    LPIPS   texture  floaters
#   sh3_viewdep_refix 25.435  0.7498  0.3990  0.4504   0.942%   (both priors on)
#   sh3_nonormal      25.828  0.7632  0.3734  0.4854   1.193%
#   dssim05           25.519  0.7709  0.3614  0.4959   0.958%
#   noprior           26.083  0.7693  0.3611  0.5005   1.726%   (both priors off, dssim 0.2)
#
# noprior is the best photometric result so far and the first past 26, but it carries 1.8x the
# floaters of dssim05. The two have never been combined, and raising the SSIM weight is the only
# change measured so far that REDUCES floaters (1.193% -> 0.958% at fixed count), so it is the
# targeted answer to what noprior costs.
#   26+ with floaters near 1%  -> this is the recipe, and 4x4 can start
#   26+ with floaters near 1.7% -> the gain is bought with geometry; decide explicitly whether to
#       pay, and check surf_normal before shipping it
#   below 26                    -> the two changes overlap; keep noprior and drop the dssim raise
# ⚠ Photometric metrics cannot see geometry. Removing BOTH geometric priors is where that blind
#   spot bites hardest -- floater fraction from task_test.sh is part of the verdict, not a footnote.
#
