...
echo "[dynk] b12 SB@2M dynamic-K@1.2M start $(date +%F_%T)" >> $PROG
rm -rf outputs/mcmc_sb_dynk_b12_cap1p2m
conda run -n gspl python -u main.py fit \
  ...
  --model.strip_max 8 \
  -n mcmc_sb_dynk_b12_cap1p2m > logs/mcmc_sb_dynk_b12_cap1p2m.log 2>&1
rc=$?
psnr=$(grep -oE 'val/psnr: [0-9.]+' outputs/mcmc_sb_dynk_b12_cap1p2m/blocks/block_12/results.txt 2>/dev/null | head -1)
kmax=$(grep -oE '\-> K=[0-9]+' logs/mcmc_sb_dynk_b12_cap1p2m.log 2>/dev/null | grep -oE '[0-9]+' | sort -n | tail -1)
