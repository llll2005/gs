#!/bin/bash
# ⚠⚠ 臨時孿生檔 #3（2026-09-13）。lab 的 task_cmp.sh 與 task_cmp2.sh **都有執行中的讀者**，
#   而 Jupyter 的 put 是**就地覆寫（同 inode，已實測）** => 覆寫會靜默截斷那些跑次。
# ⚠ 清理：lab 閒置時把本檔覆蓋回 task_cmp.sh，並刪掉 task_cmp2.sh / task_cmp3.sh。
...
case "$ARM" in
  ...
  cb25cost)   [ "${B0:-0}" -gt 0 ] || { echo "⛔ block $BLK 沒有標定過的 B0"; exit 2; }
              EXTRA=(--model.density.init_args.cost_budget $((B0 / 4))
                     --model.density.init_args.cost_add_densify 4.0
                     --model.density.init_args.cost_budget_report 500);          EXP=2 ;;
  *) echo "⛔ 未知的 arm：$ARM（base|costdir|costdir_cal|costtaming|dssim05|both|cb50|cb25|cb50cost|cb25cost|trimvpc|trimvpc05）"; exit 2 ;;
esac
