#!/bin/bash
# ⚠⚠ 這是 `task_cmp.sh` 的**臨時孿生檔**（2026-09-13 建立）。
#   建立原因：lab 上有兩個跑次（cs_dssim05 / cs_both）**正在執行 task_cmp.sh 本身**，
#   而覆寫執行中的 bash 腳本會讓它按舊的位元組偏移續讀 => 靜默截斷剩下的內容且 rc=0
#   （本專案已量測過這個行為）。本檔內容 = 本機版 task_cmp.sh 逐行相同，只是換個檔名。
# ⚠ 清理：那兩個跑次結束後，把本檔覆蓋回 scripts/lab/task_cmp.sh 並刪除本檔。
#   在那之前 **lab 的 task_cmp.sh 是舊版**（沒有 trimvpc / costdir_cal，且 costdir 寫 4.0）。
