...
#
# sched30 的 30k 是在 **w=0**（沒有 |g| 導向）時調出來的。w=2 讓 densify 階段
# 產出更有價值 => 最佳停止點可能後移。這是唯一一個「舊參數可能因新機制而過時」的位置。
# 對照 = agd2_b12（26.5602）。唯一變數 = densify_until 30k -> 36k。
# 為什麼相信還有空間（§11.30 實測）：
...
#
# 判準：`tools/cmp_runs.py agd_b12 agd2du36_b12`（最後 4 個 val 點平均），
#       外加 `tools/veil_detect.py agd_b12:12 agd2du36_b12:12` 看糊掉%（agd_b12 = 46.17%）。
...
conda run -n gspl --no-capture-output python -u main.py fit \
  ...
  --model.density.init_args.densify_until_iter 36000 \
