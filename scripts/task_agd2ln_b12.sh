#!/bin/bash
# ★★★★★ 把 CityGaussianV2 有、我方關掉的幾何正則項加回來：法線一致性（lambda_normal=0.0125）
#
...
#
# 判準：`tools/cmp_runs.py agd_b12 agd2ln_b12`（最後 4 個 val 點平均），
#       外加 `tools/veil_detect.py agd_b12:12 agd2ln_b12:12` 看糊掉%（agd_b12 = 46.17%）。
#       ⚠ 對照是 **agd_b12（26.432）**，不是 sched30_b12 —— 問的是「更強有沒有更好」。
set -u
cd "$(dirname "$0")/.." || exit 1
export PYTORCH_CUDA_ALLOC_CONF=max_split_size_mb:128
rm -rf outputs/agd2ln_b12
conda run -n gspl --no-capture-output python -u main.py fit \
  ...
  --model.metric.init_args.lambda_normal 0.0125 \
