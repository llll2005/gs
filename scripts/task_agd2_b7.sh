#!/bin/bash
# ★★★★★ 跨塊驗證：absgrad_densify **2.0** 換到 block_7（w=2 進配方的必要條件）
#
# agd2_b12 在 b12 三指標全贏 w=1.0（+6.4/+25.5/+8.3 sd），但那是 n=1 單塊。
# ⚠ notrim2 就是 b12 贏、b7 反轉 => 未經 b7 驗證的配方不可採用。
# 對照 = agd_b7（25.28，w=1.0）。唯一變數 = 強度。
# 為什麼相信還有空間（§11.30 實測）：
...
#
# 判準：`tools/cmp_runs.py agd_b12 agd2_b7`（最後 4 個 val 點平均），
#       外加 `tools/veil_detect.py agd_b12:12 agd2_b7:12` 看糊掉%（agd_b12 = 46.17%）。
#       ⚠ 對照是 **agd_b12（26.432）**，不是 sched30_b12 —— 問的是「更強有沒有更好」。
set -u
cd "$(dirname "$0")/.." || exit 1
export PYTORCH_CUDA_ALLOC_CONF=max_split_size_mb:128
rm -rf outputs/agd2_b7
