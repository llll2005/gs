...
PLY=data/matrix_city/aerial/train/block_all/depth_init/block_7.ply

echo "[freeze] K8 not preset cap restrict start $(date +%F_%T)" >> $PROG
rm -rf outputs/freeze_b7_K8_v2
python -u probes/p5_cost_aware/train_p5.py --block_id 7 --init_ply $PLY \
  --max_steps 10000 --refine_start 500 --refine_every 150 \
  --refine_stop_frac 0.6 --val_every 5000 --cost_aware on --intersection_budget 1e9 \
  --K_strips 8 --out outputs/freeze_b7_K8_v2 > logs/freeze_b7_K8_v2.log 2>&1
rc=$?
psnr=$(grep -oE "VAL step 10000: psnr [0-9.]+" logs/freeze_b7_K8_v2.log | tail -1)
final=$(grep -oE "n [0-9,]+ peakVRAM [0-9.]+G" logs/freeze_b7_K8_v2.log | tail -1)
echo "[freeze] K2-v2-b7 done rc=${rc} ${psnr} ${final} $(date +%F_%T)" >> $PROG
