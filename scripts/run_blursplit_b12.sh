#!/bin/bash
# Blur split: pick densification hosts by under-reconstruction, not by opacity.
#
...
#
# weight 2.0: a primitive one threshold over gets 3x the selection weight of an equally opaque one,
# clamped at 33x so a single blob cannot monopolise a densification step.
#
...
conda run -n gspl --no-capture-output python -u main.py fit \
  ...
  --model.density.init_args.blur_split_weight 2.0 \
