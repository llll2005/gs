#!/bin/bash
# ★★★★★ 找 absgrad_densify 強度的轉折點：w=3.0
#
# w=1.0 -> 2.0 三指標全贏，但紋理比 -2.7sd（首次出現代價跡象）=> 峰值可能就在附近。
# w=3 若開始掉，代表 w=2 在峰值附近，這條線收斂，不必再往上試。
#
# 為什麼相信還有空間（§11.30 實測）：
...
#
# 判準：`tools/cmp_runs.py agd_b12 agd3_b12`（最後 4 個 val 點平均），
#       外加 `tools/veil_detect.py agd_b12:12 agd3_b12:12` 看糊掉%（agd_b12 = 46.17%）。
...
rm -rf outputs/agd3_b12
conda run -n gspl --no-capture-output python -u main.py fit \
  --config configs/mcmc_2dgs_60k_sh3_aggr17_aerial.yaml \
  --model.initialize_from data/matrix_city/aerial/train/block_all/depth_init/block_12.ply \
  --data.parser.block_id 12 \
  --model.density.init_args.cap_max 2600000 \
  --model.density.init_args.absgrad_densify 3.0 \
